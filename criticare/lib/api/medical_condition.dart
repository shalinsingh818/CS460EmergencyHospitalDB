...
class MedicalConditionApi {
    ...
    static Future<List<Condition>> fetchPatientConditions(int? patientId) async {
        final response = await http.get(Uri.parse('http://127.0.0.1:5000/diagnosePatient?patient=${patientId}'));
        if (response.statusCode == 200) {
            final List<Condition> conditions = conditionFromJson(response.body);
            return conditions;
        } else {
