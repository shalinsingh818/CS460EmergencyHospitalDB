...
class MedicationApi {
    ...
    static Future<List<Medication>> fetchPatientMedications(int? patientId) async {
        final response = await http.get(Uri.parse('http://127.0.0.1:5000/prescribeMedication?patient=${patientId}'));
        if (response.statusCode == 200) {
            final List<Medication> medications = medicationFromJson(response.body);
            return medications;
        } else {
