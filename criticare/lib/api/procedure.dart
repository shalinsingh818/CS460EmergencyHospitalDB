...
class ProcedureApi {
    ...
    static Future<List<Procedure>> fetchPatientProcedures(int? patientId) async {
        final response = await http.get(Uri.parse('http://127.0.0.1:5000/patientProcedure?patient=${patientId}'));
        if (response.statusCode == 200) {
            final List<Procedure> procedures = procedureFromJson(response.body);
            return procedures;
        } else {
            throw Exception('Failed to load album');
        }
