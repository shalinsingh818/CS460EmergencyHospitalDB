...
class IntakeApi {
    ...
    static Future<PatientBill> fetchIntakePatientBill(int? patientId) async {
        final response = await http.get(Uri.parse('http://127.0.0.1:5000/patientBill?patient=${patientId}'));
        if (response.statusCode == 200) {
            // If the server did return a 200 OK response,
            // then parse the JSON.
            return PatientBill.fromJson(jsonDecode(response.body));
        } else {
            // If the server did not return a 200 OK response,
            // then throw an exception.
            throw Exception('Failed to load album');
